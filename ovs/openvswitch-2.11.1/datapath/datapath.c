...
atomic_t hit_fail_kernal_table = ATOMIC_INIT(0);
...
void ovs_dp_process_packet(struct sk_buff *skb, struct sw_flow_key *key)
{
	...
	int error_printk=printk("Gary:%lu " NIPQUAD_FMT " " NIPQUAD_FMT " %lu %lu %lld %d %lld %ld %ld %ld %ld %ld %ld %ld %ld %ld\n",\
...
para:
	 ...
	 * struct genl_info - receiving information
	 成员
	 * @snd_seq: 发送序列号（区分不同数据包）
	 * @snd_portid: netlink portid of sender
	 * @nlhdr: netlink message header
	 * @genlhdr: generic netlink message header
	 * @userhdr: user specific header
	 * @attrs: netlink attributes指针数组
	 * @_net: network namespace
 
return:返回零表示成功,失败的时候返回linux中 <errno.h>中的相应的错误类型，一般错误习惯返回负值，因此常常在这些错误类型前面添加负号。
descr:此函数是netlink命令OVS_FLOW_CMD_SET的一个响应函数，当内核态接收到用户态发送OVS_FLOW_CMD_SET命令后，就会执行该函数。
*/
...
return:返回零表示成功,失败的时候返回linux中 <errno.h>中的相应的错误类型，一般错误习惯返回负值，因此常常在这些错误类型前面添加负号。
descr:此函数是netlink命令OVS_FLOW_CMD_GET的一个响应函数，当内核态接收到用户态发送OVS_FLOW_CMD_GET命令后，就会执行该函数。
*/
...
return:返回零表示成功,失败的时候返回linux中 <errno.h>中的相应的错误类型，一般错误习惯返回负值，因此常常在这些错误类型前面添加负号。
descr:此函数是netlink命令OVS_FLOW_CMD_DEL的一个响应函数，当内核态接收到用户态发送OVS_FLOW_CMD_DEL命令后，就会执行该函数。
*/
...
struct genl_family dp_vport_genl_family __ro_after_init = {
	.hdrsize = sizeof(struct ovs_header), //用户自定义头部长度
	.name = OVS_VPORT_FAMILY,
	.version = OVS_VPORT_VERSION,
	.maxattr = OVS_VPORT_ATTR_MAX, //最大的attribute数量
	.netnsok = true, //是否支持处理network namespace
	.parallel_ops = true, //操作是否可以并行调用
	.ops = dp_vport_genl_ops, //支持的操作
	.n_ops = ARRAY_SIZE(dp_vport_genl_ops),  //支持的操作数
	.mcgrps = &ovs_dp_vport_multicast_group, //该family使用的多播组
	.n_mcgrps = 1,
	.module = THIS_MODULE, //所属于的模块
};
