    ...
    public NodeConnectorRef getNodeConnector(InstanceIdentifier<Node> nodeInsId, MacAddress macAddress) {
        ...
        try {
            ...
            if (dataObjectOptional.isPresent()) {
                ...
                LOG.debug("JBH: In getNodeConnector: Looking address{} in node : {}", macAddress, nodeInsId);
                for (NodeConnector nc : node.getNodeConnector()) {
                    ...
                    LOG.debug("JBH: In getNodeConnector: Looking address{} in nodeconnector : {}", macAddress, nc.getKey());
                        ...
                        for (Addresses add : addressesList) {
                            if (macAddress.equals(add.getMac())) {
                                if (add.getLastSeen() > latest) {
                                    ...
                                    LOG.debug("JBH: getNodeConnector: Found address{} in nodeconnector : {}", macAddress, nc.getKey());
                                    break;
                                }
                            }
                        }
            ...
            LOG.error("JBH: In getNodeConnector: Failed to read nodes from Operation data store.");
            readOnlyTransaction.close();
            throw new RuntimeException("Failed to read nodes from Operation data store.", e);
        } catch (ExecutionException e) {
            LOG.error("JBH: In getNodeConnector: Failed to read nodes from Operation data store.");
            readOnlyTransaction.close();
            throw new RuntimeException("JBH: In getNodeConnector: Failed to read nodes from Operation data store.", e);
        }
        readOnlyTransaction.close();
