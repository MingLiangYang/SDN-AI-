...
void GainRecord(FILE *f,FILE *res,vector<SubRecord> Record,int Tag);//处理向量Record，计算熵值，得到最终每条记录的各项数据，并将数据存入f指向的文件中//注意：要求Record中的记录是按照time升序排列 //Tag是0代表正常流量，Tag是1代表攻击流量 
